...
#include <fcntl.h>		// The prototypes of the interface
...
Java_xxl_core_io_raw_NativeRawAccess_open(JNIEnv *env, jobject obj, jstring jfilename) {
	int jfilep; 			// int of the file/device
	...
	jfilep = open(filename,O_RDWR | O_BINARY);
	...
	length = lseek(jfilep,0,SEEK_END); // does not work with _lseeki64!
	if (length==-1) {
		// perror("lseek error"); // output textual system error message
...
Java_xxl_core_io_raw_NativeRawAccess_write (JNIEnv *env, jobject obj, jbyteArray jblock, jlong sector) {
	int jfilep = (int) getfilep(env, obj); 				// Device int
	...
	// Set position
	if (_lseeki64(jfilep,fpos,SEEK_SET)==-1) {
...
Java_xxl_core_io_raw_NativeRawAccess_read (JNIEnv *env, jobject obj, jbyteArray jblock, jlong sector) {
	int jfilep = (int) getfilep(env, obj);		// Get the device int
