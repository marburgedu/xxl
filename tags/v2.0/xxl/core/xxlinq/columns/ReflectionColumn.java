		...
		constrs: for (Constructor c : constructors) {
			...
			for (int i = 0; i < pmt.length; i++) {
				if (curPmt[i].isPrimitive()) {
					...
					/*
					 * für zahlentypen (double, float, int, long, short, char)
					 * gilt: Methode mit ... frisst ... double : alles float :
					 * alles außer double int : nur char, short und int long :
					 * alle außer double und float short : nur short char : nur
					 * char
					 */
