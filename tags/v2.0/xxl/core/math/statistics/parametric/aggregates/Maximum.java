...
public class Maximum extends AggregationFunction<Number,Number> {
	 ...
	 */
	public Number invoke(Number max, Number next) {
		return next == null ? max : max == null ? next.doubleValue() : Math.max(max.doubleValue(), next.doubleValue());
	}
}
