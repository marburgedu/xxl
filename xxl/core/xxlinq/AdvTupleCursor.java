...
public class AdvTupleCursor extends AbstractCursor<Tuple> 
	 ...
	 * 
	 */
	// TODO: SMART weitergedacht könnte außerdem bei reset von gecachetem cursor j die listen in 
	// cursorn i mit i<j gelöscht werden - solange die nicht geclont wurden...
			...
			OperatorImplementation operatorImpl, AdvTupleCursor... parents)
	{
		...
		if(parents != null && parents.length>0){
			...
			// TODO: hier könnte man ganz smarten kram machen und die strategien beider eltern
			// betrachten und die bessere (über)nehmen oder sowas
	...
	protected void internal_sort(Comparator<Tuple> comp){
		...
		// interne liste sortieren - wäre etwas schneller mit ner arraylist, da die elemente
		// in nen array kopiert, dann sortiert und dann zurück in die liste kopiert werden...
		// und das kopieren ist von array zu array schneller. (auf die idee, bei ner arraylist
		// direkt aufm internen array zu sortieren ist sun - ähhh oracle. - aber noch nicht 
		// gekommen...)
	...
	public void reset() throws UnsupportedOperationException {
		...
		if(doNotCache){
			...
			if(firstRun) { // if the cursor is reset on first run..
				...
				// FIXME: sollte man den schließen?
				// wrappedCursor.close();
				firstRun = false;
			} 
	...
	public CompositeMetaData<Object, Object> getMetaData() {
		// solange den quatsch keiner braucht, müssen wir ihn auch nicht initialisieren.
...
// ##########################################################################
// ab hier: Voodoo für "automagisches" Mapping beliebiger Klassen auf Tupel #
// ##########################################################################
...
// ######################################################################
// ab hier: Methoden für LINQ-artige Funktionalität (join, select, ...) #
// ######################################################################
