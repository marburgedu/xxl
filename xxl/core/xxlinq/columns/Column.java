...
package xxl.core.xxlinq.columns;
...
public class Column extends AbstractFunction<Tuple, Object> implements CorrTuplesReceiver {
	...
	// TODO: evtl alle Konstrkutoren private ?!? - wenn dann package protected für ColumnUtils
 ...
 ***********************************/
	...
	public AdvPredicate GT(final Column col, final Comparator cmp) {
		...
		AdvPredicate ret = new AdvPredicate() {
			// wenn man nen eigenen comparator setzen wollte, könnte man das
			// einfach hier tun und in
			// invoke würd sich nix ändern
			Comparator	comp	= cmp;
 ...
 * ***************************************************/
	...
	public Column CONCAT(final Column col, String name) {
		...
		// den alias setzen wir später in setMetaData() .. bisschen hackish,
		...
		Column ret = new Column(name, cmd) {
			...
			@Override
			// für den join-fall (columns können aus linkem oder rechtem tupel
			// sein)
			public Object invoke(Tuple left, Tuple right) {
